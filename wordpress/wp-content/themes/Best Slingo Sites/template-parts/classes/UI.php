...
class UI implements Injectable {
	...
	public function Archive( $data ): string {
		...
		$res     = "<article>
          <a href='$link'><img src='$thumbnail' width='336' height='198' alt='$alt'></a>
...
";
	...
	public function CasinoListItem( $data ): string {
		...
		$arrowImg = $hp->GetImage( 'arrowdown.png' );
		...
		if ( $rated > 3 ) {
			...
			$img      = $hp->GetImage( 'toprated' . $rated . '.png' );
		...
		<div class='col-lg-12 col-md-6 casino-list-item flex align-center justify-center d-column'>
            <div class='flex align-center d-column general'>
                <div class='info flex align-center justify-center'>
		                $topRated
		                <a href='$link' style='margin-top: 0'>
...
<!--							--><?php //if ( $rated < 3 ): ?>
<!--                                <img src='--><?//= $img ?><!--' class='rated' width='50'-->
...
<!--							--><?// endif; ?>
	...
	public function Stars( $n ): string {
		$hp        = Container::Get( 'helper' );
		$res       = "<div class='stars'>";
		$starFull  = $hp->GetImage( "starfilled.png" );
		$starEmpty = $hp->GetImage( "starempty.png" );
	...
	public function GameUI( $data, $hasButton = true, $linkImage = true ): string {
		...
		$a      = $linkImage
			? "<a href='$link'><img  style='max-width:100%' src='$thumb[0]' width='373' height='210'  alt='$alt'></a>"
			: "<a href='#' onclick='return false'><img  style='max-width:100%' src='$thumb[0]' width='373' height='210'  alt='$alt'></a>";
	...
	public function ArticleUI( $data, $showCategory = true, $linkImage = true ): string {
        ...
        <div class='col-xl-4 col-lg-6 col-12 '>
            <div style='max-width:100%' data-article-id='<?= $id ?>'
				...
				<?php if ( $linkImage ): ?>
                    <a href='<?= $link ?>' style='margin-top: 0'><img style='max-width:100%;' src='<?= $thumb[0] ?>'
                                                                      width='373' height='210' alt='<?= $alt ?>'></a>
				<?php else: ?>
                    <a href='#' onclick='return false' style='margin-top: 0'><img style='max-width:100%;'
                                                                                  src='<?= $thumb[0] ?>'
