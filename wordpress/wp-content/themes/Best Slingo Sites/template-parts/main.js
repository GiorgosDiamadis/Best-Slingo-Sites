...
class CasinoFinder {
    constructor() {
        ...
        if (window.screen.width > 1000) {
            this.currentButton = qs(".steps").children[this.currentStep - 1];
        } else {
            this.currentButton = qs(".steps-mobile").children[this.currentStep - 1];
        }

        this.buttonSteps.forEach((btn) =>
            btn.addEventListener("click", (e) => this.stepClicked(e.target))
        );
        if (window.screen.width > 1000) {
            if (this.nextButtons)
                this.nextButtons.forEach((btn) =>
                    btn.addEventListener("click", (e) => this.stepChanged())
                );
            if (this.backButtons) {
                this.backButtons.forEach((btn) =>
                    btn.addEventListener("click", (e) => this.stepChanged(false))
                );
            }
        } else {
            if (this.nextButtons)
                this.nextButtons.forEach((btn) =>
                    btn.addEventListener("click", (e) => this.stepChangedMobile())
                );
            if (this.backButtons) {
                this.backButtons.forEach((btn) =>
                    btn.addEventListener("click", (e) => this.stepChangedMobile(false))
                );
            }
        }
    }
    ...
    list.forEach((item, i) => {
        item.addEventListener("click", (e) => {
            ...
            if (item.getAttribute("data-game-id")) {
                nextStep = qs("#step-1 button[data-finder='next-step']");
            } else {
                const step = qs("#step-2");
                if (!step.classList.contains("d-none")) {

                    nextStep = step.querySelector("button[data-finder='next-step']");
                    back = step.querySelector("button[data-finder='back-step']")
                } else {
                    nextStep = qs("#step-3 button[data-finder='find-casino']");
                    back = qs("#step-3 button[data-finder='back-step']")
                }
            }
...
cf && (sticky.style.display = "none", document.addEventListener("scroll", a => {
